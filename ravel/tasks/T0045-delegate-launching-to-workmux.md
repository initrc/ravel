...
- Selecting derived `ready-to-merge` executes `workmux add <task-branch> --open-if-exists` without a prompt so the existing worktree can be inspected or its interrupted integration resumed.
...
- A `ready-to-merge` fallback prints its branch and registered worktree path without generating or copying a new implementation prompt.
...
- Tests use fake executables and clipboard boundaries and cover new and existing workmux delegation, prompt-variant selection, direct prompt injection without clipboard writes, `ready-to-merge` reopening, safe argument passing, inherited output and exit status, every fallback, and copy failure.
